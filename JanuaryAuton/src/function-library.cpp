...
void Robot::turnDegrees(double degrees, double degreesPerSecond)
{
  if(accella.rotation() < degrees)
  {
    turnRight(degrees, degreesPerSecond);
  }
  else if(accella.rotation() > degrees)
  {
    turnLeft(degrees, degreesPerSecond);
  }
  else
  {
...
void Robot::turnDegrees(double degrees, double degreesPerSecond, bool doCoast)
{
  if(doCoast)
  {
    bLeftDrive.setStopping(coast);
...
void Robot::turnLeft(double degrees, double degreesPerSecond)
{
  ...
  while(accella.rotation() > degrees)
  {
    vex::task::sleep(20);
  }
  bLeftDrive.stop();
...
void Robot::turnRight(double degrees, double degreesPerSecond)
{
  bLeftDrive.spin(forward, degreesPerSecond, dps);
  fLeftDrive.spin(forward, degreesPerSecond, dps);
  bRightDrive.spin(forward, -degreesPerSecond, dps);
...
void Robot::turnDistance(double ticksL, double speedL, double ticksR, double speedR)
{
  ...
  //in the event that the left wheels have to go further or faster than the right, wait for them.
  if((!bLeftDrive.isDone()) || (!fLeftDrive.isDone()))
  {
